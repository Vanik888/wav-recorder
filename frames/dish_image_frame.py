...
class DishImageFrame(Frame, CommonFrameMixin):
    def __init__(self, *args, **kwargs):
        ...
        self.dish_name_lbl = Label(self, text=self.get_name_formated_by_lines(dish_lbl_txt_length, self.dish.name), font=('Helvetica', 16, 'bold'), anchor='n')
        ...
        self.dish_price_lbl = Label(self, text=self.get_name_formated_by_lines(dish_lbl_txt_length, self.get_price_str()), anchor='n', font=('Helvetica', 14))
        self.count_lbl = Label(self, text=self.get_name_formated_by_lines(5, '0'), font=('Helvetica', 44, 'bold'), anchor='center')
        self.dish_img_lbl = Label(self, image=self.image, compound='right')
    ...
    def place_content(self):
        ...
        price_lbl_size = {'height': 65, 'width': 250}
        count_lbl_size = {'height': 80, 'width': 250}
    ...
    def get_price_str(self):
        ...
        total_price = str(self.dish.count*self.dish.price)
        price_str = 'Цена: %s Руб\n Итого: %s Руб' % (price, total_price)
        return price_str

    def update_dish_lbl(self):
        price_txt = self.get_price_str()
        self.dish_price_lbl.config(text=price_txt)
        self.count_lbl.config(text=self.get_name_formated_by_lines(5, str(self.dish.count)))
