...
class TopMenu():
    def __init__(self, *args, **kwargs):
        ...
        self.frame = Frame(self._root, **self._frame_size)

        self.dishes_btn = Button(self._root, text='Выбор блюд', image=self.image, compound='right')
        self.dishes_btn.bind('<Button-1>', self.choose_dishes_ev)

        self.drinks_btn = Button(self._root, text='Выбор напитка', image=self.image, compound='right')
        self.drinks_btn.bind('<Button-1>', self.choose_drinks_ev)

        self.service_btn = Button(self._root, text='Сервис', image=self.image, compound='right')
        self.service_btn.bind('<Button-1>', self.choose_service_ev)

        self.bill_btn = Button(self._root, text='Оплата', image=self.image, compound='right')
